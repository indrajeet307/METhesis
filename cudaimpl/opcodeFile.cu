...
int getClassId( char * cls)
{
    if ( strcmp( cls, "MALWARE") == 0 ) return 0;
    if ( strcmp( cls, "BENIGN") == 0 ) return 1;
    return -1;
}

s_fileProp* createFileNode( char *filename, int filesize, char *data_type, char *cls, int numopcode, int totalopcodes)
{
...
int readOpcodeFile( char* fname, s_trie** opcodelist)
{
    ...
    while( !feof( fp ) )
    {
        ...
        //printf(" %s", buff);
        opcode = strtok( buff, "\n" );
        if ( findWord( opcodelist, opcode, strlen(opcode) ) == -1)
            addWord(opcodelist, opcode, strlen(opcode) );
        //printf("%s", opcode);
        numopcodes++;
    }
    fclose( fp );
...
int readCSVFile( char* fname, int columns, s_files ** p_files, int *groupCount, s_filelist **trainlist, s_filelist **testlist)
{
    ...
    char  *cls    ;//= (char*) malloc (sizeof(char)*16);
...
#endif
                    s_fileProp *tempfile = createFileNode( filename, size, data_set, cls, numopcode, columns);
